...
void MetalRenderer::draw_execute(uint32 baseVertex, uint32 baseInstance, uint32 instanceCount, uint32 count, MPTR indexDataMPTR, Latte::LATTE_VGT_DMA_INDEX_TYPE::E_INDEX_TYPE indexType, bool isFirst)
{
	...
	*/
    ...
    // RECTS is emulated too. It used to be excluded here on the strength of a device log
    // that recorded zero RECTS draws against nineteen thousand geometry-shader ones. That
    // measurement was taken while shader compilation was failing, and the failures masked
    // the drops: with shaders fixed, the same title reported 39,098 RECTS drops against
    // 4,903 geometry-shader ones. RECTS carries the copy and post-processing passes, which
    // is why dropping it leaves correct textures with the colour crushed out of them.
    //
    // It still has no LatteDecompilerShader - its stage is generated by
    // rectsEmulationGS_generate rather than decompiled - so the layout the dispatch needs
    // is carried on the pipeline object instead.
    const bool emulateGeometryShader = usesGeometryShader && !m_supportsMeshShaders && m_emulateGeometryShader;
    ...
    // Fetched before the emulated stages rather than after them, for two reasons: a RECTS
    // draw needs the generated geometry shader that hangs off this object, and a pipeline
    // that failed to build should skip the dispatches altogether rather than do the work
    // and then find there is nothing to draw with.
	...
	if (emulateGeometryShader)
	{
		...
		// A RECTS stage is generated, not decompiled, so it is reached through the pipeline
		// object rather than through geometryShader->shader, which does not exist for it.
		auto geometryShaderMtl = emulateRects ? pipelineObj->m_rectsEmulationShader
		                                     : static_cast<RendererShaderMtl*>(geometryShader->shader);
		if (!geometryShaderMtl)
		{
			// The generated stage failed to build. Skip rather than dispatch against null.
		...
		// A rectangle always expands to exactly two triangles, written out already
		// expanded, so its vertex count is fixed rather than bounded by MTL_GS_MAX_VERTICES.
		const uint32 gsVertexStride = emulateRects ? pipelineObj->m_rectsEmulationVertexStride
		                                          : geometryShader->mtlGsVertexStride;
		const uint32 verticesPerEmulatedPrimitive = emulateRects ? 6u : MTL_GS_MAX_VERTICES;
		...
		const size_t outBytes = (size_t)primitiveCount * verticesPerEmulatedPrimitive * gsVertexStride;
		...
		if (emulateRects)
		{
			// The generated kernel samples nothing and reads no uniforms - it only reshapes
			// the payload - so there are no stage resources to bind. Its two buffers sit at
			// fixed indices because it is generated here rather than assigned by the
			// decompiler, and it needs no primitive-count buffer: a rectangle always emits
			// exactly two triangles, so there is nothing to clip away downstream.
			computeCommandEncoder->setBuffer(m_gsPayloadBuffer, 0, 0);
			computeCommandEncoder->setBuffer(m_gsOutBuffer, 0, 1);
		}
		else
		{
		...
		// RECTS is exact rather than worst-case: two triangles, six vertices, every slot
		// written, nothing for the passthrough to discard.
		gsDrawVertexCount = emulateRects ? (primitiveCount * 6u)
		                                 : (primitiveCount * primsPerInvocation * verticesPerOutPrimitive);
		if (emulateRects)
			gsOutMtlPrimitiveType = MTL::PrimitiveTypeTriangle;
